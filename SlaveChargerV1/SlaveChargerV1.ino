...
int incomingByte = 0;
int state;
unsigned long tlast, tcan, tstate, tboot = 0;
...
void setup()
{
  ...
  if (parameters.version != EEPROM_VERSION)
  {
    ...
    parameters.canControl = 3; //0 disabled can control, 1 master, 2 slave
    ...
    case 1://Charger on
      ...
      else
      {
        ...
        if (millis() - tstate > 500){
          Serial.print(" State Set back to 0 inside of State switch because Charger Enable not high. line 414\n\n");
          tstate = millis();
        }
      }
      break;
