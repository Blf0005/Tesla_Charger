///*********************************************************************************************************************************************************************
///
/// Premergy, Inc ("COMPANY") CONFIDENTIAL
/// Unpublished Copyright (c) 2017-2018 Premergy, Inc, All Rights Reserved.
///
...
///
/// The copyright notice abov\e does not evidence any actual or intended publication or disclosure  of  this source code, which includes  
...
int incomingByte = 0;
int state;
unsigned long tlast, tcan, tstate, tboot, lastDCDC = 0;
...
void setup()
{
  ...
  Timer4.attachInterrupt(Control_msgs).start(250000); // charger messages every 250ms
  ...
  if (parameters.version != EEPROM_VERSION)
  {
    ...
    parameters.currReq = 3000; //max input limit per module 1500 = 1A
...
void loop()
{
  ...
  if(DCDC_Flag == false){
    if ((millis() - lastDCDC) > 21600000){
      digitalWrite(DIG_OUT_1, HIGH); //Turn on DCDC for one hour
      DCDC_Flag = true;
      lastDCDC = millis();
    }
  }
  else{
    if((millis() - lastDCDC) > 3600000){
      digitalWrite(DIG_OUT_1, LOW); //DCDC has been on for an hour. Turn it off.
      DCDC_Flag = false;
    }
  }
      ...
      case 's'://s for start AND stop
        if (Serial.available() > 0)
        {
          ...
          if (state == 0)
          {
            ...
            tboot = millis();
            Serial.print(" State Set to 2 inside of Serial switch. line 251\n\n");
          }
          if (state == 1)
          {
            state = 0;// initialize modules
            Serial.print(" State Set to 0 inside of Serial switch. line 257\n\n");
          }
        }
        break;
  ...
  if (parameters.canControl > 1)
  {
    ...
    {
      state = 0;
      Serial.print(" State Set to 0 inside of Can Time out. line 336\n\n");
      Serial.println();
  ...
  switch (state)
  {
    ...
    case 1://Charger on
      ...
      else
      {
        ...
        if (millis() - tstate > 500){
          Serial.print(" State Set back to 0 inside of State switch because bChargerEnabled was false. line 414\n\n");
          tstate = millis();
        }
      }
      break;
    ...
    case 2:
      ...
      {
        state = 1;
        Serial.print(" State Set to 1 inside of State switch case 2. line 414\n\n");
      }
    ...
    if (debug != 0)
    {
      ...
      if (bChargerEnabled)
      {
        ...
        for (int x = 0; x < 3; x++)
        {
          ...
          Serial.print("  AC cur: ");
          Serial.print((accur[x] * 0.006666), 2);
          Serial.print("  ");
  ...
  if (Proximity == Connected) //check if plugged in
  {
    ...
    if (modulelimcur > 1400) // one amp or more active modules
    {
      ...
      digitalWrite(DIG_OUT_2, HIGH); //Turn on Cooling Pump
      digitalWrite(DIG_OUT_1, HIGH); //Turn on DC/DC Converter
    }
    /*
  ...
  else // unplugged or buton pressed stop charging
  {
    state = 0;
    digitalWrite(EVSE_ACTIVATE, LOW);
    digitalWrite(DIG_OUT_2, LOW); //Shut down Cooling Pump
    digitalWrite(DIG_OUT_1, LOW); //Shut down DC/DC
    if (millis() - tstate > 500){
     // Serial.print(" State Set to 0 inside of evseRead because unplugged or button press. line 615\n\n");
      tstate = millis();
    }
  }
}
...
void Control_msgs(){
  ...
  if (parameters.canControl == 1)
  {
    ...
    outframe.data.bytes[7] = Proximity;
...
void canextdecode(CAN_FRAME & frame)
{
    ...
    if (ElconControlID == frame.id) //Charge Control message
    {
      ...
      if (frame.data.bytes[4] & 0x01 == 1)
      {
        if (state == 0)
        {
          ...
          tboot = millis();
          if (millis() - tstate > 500){
           // Serial.print(" State Set to 2 inside of Canextdecode. line 1118\n\n");
            tstate = millis();
          }
        }
      }
      else
      {
        state = 0;
        if (millis() - tstate > 500){
         // Serial.print(" State Set to 0 inside of Canextdecode. line 1125\n\n");
          tstate = millis();
        }
      }
      if (candebug == 1)
  ...
  if (parameters.canControl == 3)
  {
    ...
    {
      //Serial.print(" Received Control Frame\n\n");
      if (frame.data.bytes[0] & 0x01 == 1)
      {
        if (state == 0)
        {
          ...
          tboot = millis();
          if (millis() - tstate > 500){
            //Serial.print(" State Set to 2 inside of Canextdecode. line 1150\n\n");
            tstate = millis();
          }
        }
      }
      else
      {
        state = 0;
        if (millis() - tstate > 500){
          //Serial.print(" State Set to 0 inside of Canextdecode. line 1157\n\n");
          tstate = millis();
        }
      }
